...
jq -e 'all(.[]; .unknown == null or (.unknown as $u | ([$u.stage,$u.step,$u.reason,$u.unknown_class,$u.next_operation] | all(.[]; type == "string" and length > 0)) and ($u.blocked_by | length) > 0))' <(jq -s 'map(fromjson)' "$violations") >/dev/null
